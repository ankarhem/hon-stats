...
- `Partial("_Name")` from a PageModel only searches the page's own folder, `/Pages`, and `/Pages/Shared` — NOT `Partials/` subfolders. Views under e.g. `Pages/Players/Partials/` need the explicit path (`Partial("Players/Partials/_MatchesRows", …)`) or the handler 500s at runtime (broke infinite scroll silently; builds fine).
